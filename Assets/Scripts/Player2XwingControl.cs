...
public class Player2XwingControl : MonoBehaviour {
    ...
    public void Init()
    {
        ...
        if (GameManager.Level == 11)
            ...
            lives = GameManager.Instance.player2Lives;
        if (lives < 0)
            lives = 0;
    ...
    void Update()
    {
        ...
        // Pause / Unpause game
        if (Input.GetButtonDown("P2ButtonStart"))
        {
            LevelManager.Instance.PauseOrUnpause();
        }

        // change target
        if (Input.GetButtonDown("P2ButtonR1"))
        {
            // check the player has missiles and the game isn't paused
        ...
        // toggle S folis
        if (Input.GetButtonDown("P2ButtonRT") && !LevelManager.isPaused)
        {
            if (sFoilClosed)
            {
        ...
        // toggle manual/auto fire mode
        if (Input.GetButtonDown("P2ButtonRS") && LevelManager.Instance.GMState.ToString() == "GamePlay" && !LevelManager.isPaused)
        {
            if (!manualFireMode)
            {
                StopLasersFiringAuto();
        ...
        // manual fire
        if (Input.GetButtonDown("P2ButtonA") && !sFoilClosed && !LevelManager.isPaused)
        {
            if (manualFireMode && canFireManual)
            {
                canFireManual = false;
                // get laser linking
                if (weaponsLinked == 0)
        ...
        if (Input.GetButtonDown("P2ButtonX"))
        {
            // check if player has counter measure and the games not paused
        ...
        // torpedo linking
        if (Input.GetButtonDown("P2ButtonLT") && !LevelManager.isPaused)
        {
        ...
        // use bomb
        if (Input.GetButtonDown("P2ButtonY"))
        {
        ...
        if (Input.GetButtonDown("P2ButtonB"))
        {
        ...
        if (Input.GetButtonDown("P2ButtonL1") && LevelManager.Instance.GMState.ToString() == "GamePlay" && !LevelManager.isPaused)
        {
            weaponsLinked++;
            if (weaponsLinked == 3)
                weaponsLinked = 0;
        ...
        float x = Input.GetAxisRaw("P2Horizontal");// value will be -1, 0, 1 (left, no input, right)
        float y = Input.GetAxisRaw("P2Vertical");// (down, no input, up)
    ...
    public void TakeDamage(int damage)
    {
        ...
        {
            // is no shields, take health damage
            if (shieldHealth <= 0)
            {
                health -= damage;
            }
            // is shields, take shield damage, if there is excess damage, take that as health damage
            else
            {
                shieldHealth -= damage;
                UpdateShieldSprite(shieldHealth);
    ...
    void LoseALife()
    {
        ...
        if (lives >= 0)
        {
            ...
            // reset player position
            MoveToStartPosition();
            // reset lasers back to starting power level
            LaserPowerUpLevel = 1;
            // reload bomb and update UI?
            bombLoaded = true;
            BombUI.gameObject.SetActive(true);
            // set shield health and update shield sprite
            shieldHealth = Rules.GetXwingStartingShields();
            UpdateShieldSprite(shieldHealth);
            // set starting health
            health = Rules.GetXwingStartingHealth();
            // set starting torpedos and update UI
            torpedos = Rules.GetXwingStartingTorpedos();
            ChangeTorpedoUI();
        ...
        // if lives 0 or less, change state to game over, disable player ship
        if (lives < 0)
        {
            LevelManager.playersRemaining--;
            // need to stop lasers firing here
            StopLasersFiringAuto();
    ...
    public void MoveToStartPosition()
    {
        ...
        // x is equal to half topRightOfScreen.x
        // y is equal to half ".y * -1

        transform.position = new Vector2(topRightOfScreen.x / 2, (topRightOfScreen.y / 2) * -1);
    }
    ...
    {
        GameManager.Instance.player2Lives = lives;
    }
